...
# 热脉冲函数来自苏联《Iadernoe oruzhie》第 4 版（1987）第 49-50 页。
#
# 原图实际上是把两套刻度和 9 条曲线叠在一张斜向列线图上。
...
def _reverse_thermal_slope(y, actual_yield):
    """_thermal_slope 的逆函数，返回苏联原始列线图未归一化刻度上的 y 值。"""
...
def _reverse_soviet_ground_thermal(impulse, actual_yield, visibility):
    """按能见度编号反算苏联地爆热通量对应的距离。"""
    return _reverse_soviet_air_thermal(_ground_to_air(impulse), actual_yield, visibility)

# 容易让人困惑的是，原始列线图和上面函数里使用的 [1-9] 编号，
# 实际上和国际能见度代码正好相反。
# -----------------+------------------+---------------------+-----------------------
# 描述             | 国际能见度代码   | 模型输入（< 100kT   | 模型输入（空爆 >100kT）
#                  |                  | 及地爆）            |
# -----------------+------------------+---------------------+-----------------------
# "空气清澈"       | 9                | 1                   | 1
# "极轻薄霾"       | 7                | 3                   | 2
...
def _ivc_to_model_input(ivc, actual_yield, burst_height):
    """把国际能见度代码 [1-9] 转换为苏联热脉冲模型所使用的编号。"""
    if not 1 <= ivc <= 9:
        raise ValueOutsideGraphError(ivc)
    if ivc == 9:
        return 1
    else:
        groundburst = 10 - ivc
...
# 这里没有为热通量补充单位换算，因为几乎只会用到 Cal/cm^2。

def soviet_air_thermal(y, r, h, visibility, yunits='kT', dunits='m'):
    """按《Iadernoe oruzhie》第 4 版（1987）的方法，估算当量 y 的空爆在
    距离 r 处产生的热通量。

    `visibility` 使用国际能见度代码（IVC）表示，1 为浓雾，9 为空气清澈。
    """
    slant_range = convert_units(r, dunits, 'km')
    yld = convert_units(y, yunits, 'kT')
    _require_positive(yld, 'yield')
    _require_nonnegative(h, 'height')
    # 爆高这里只用于判断能见度编码。
...
def soviet_ground_thermal(y, r, h, visibility, yunits='kT', dunits='m'):
    """按《Iadernoe oruzhie》第 4 版（1987）的方法，估算当量 y 的地爆在
    距离 r 处产生的热通量。
...
def r_soviet_air_thermal(y, fluence, h, visibility, yunits='kT', dunits='m'):
    """按《Iadernoe oruzhie》第 4 版（1987）的方法，反算空爆在何距离处会
    出现给定热通量。
    ...
    yld = convert_units(y, yunits, 'kT')
    _require_positive(yld, 'yield')
    _require_positive(fluence, 'fluence')
    _require_nonnegative(h, 'height')
    v = _ivc_to_model_input(visibility, yld, h)
...
def r_soviet_ground_thermal(y, fluence, h, visibility, yunits='kT', dunits='m'):
    """按《Iadernoe oruzhie》第 4 版（1987）的方法，反算地爆在何距离处会
    出现给定热通量。
